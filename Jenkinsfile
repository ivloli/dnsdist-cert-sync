pipeline {
    ...
    parameters {
        string(name: 'CONFIG_YAML_ID', defaultValue: 'dnsdist-cert-sync-config', description: 'Jenkins Managed File ID for config.yaml')
        ...
        string(name: 'DEPLOY_CREDENTIALS_ID', defaultValue: 'dnsdist-cert-sync-ssh', description: 'Jenkins SSH credentials ID')
        ...
        string(name: 'DEPLOY_DIR', defaultValue: '/opt/dnsdist-cert-sync/releases', description: 'Remote directory to upload artifact')
        ...
        string(name: 'REMOTE_SERVICE', defaultValue: 'dnsdist-cert-sync', description: 'Systemd service name to restart')
    }
